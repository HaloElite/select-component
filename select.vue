<style lang="scss" scoped>
    ...
    <div :id="elementID" role="button" class="tw-relative tw-w-full tw-outline-none tw-select-none tw-border border-gray-02 tw-cursor-pointer tw-box-border tw-rounded-md focused" tabindex="0" @keydown.stop="arrowNavigation($event)">
        ...
        <div class="tw-flex tw-h-8 tw-justify-start tw-items-center bg-base tw-select-none color-primary sub-text tw-rounded-md" :class="{'border-error': hasError && !open, 'border-highlight': open, 'tw-bg-gray-50': deactivated}" @click="openCloseSelect($event)">
            <div class="tw-flex-grow tw-w-8/12 tw-rounded-l-sm tw-min-w-100" :class="{ 'open': open}">
                <template v-if="!multiselect">
                    <div class="tw-px-2 tw-overflow-hidden tw-overflow-ellipsis">
                ...
                <template v-if="multiselect">
                    <div v-if="multiSelectIndices.length > 0 && !noValueSelected">
                        <template v-for="(idx, index) in multiSelectIndices" :key="idx">
                            <div v-if="index < 2" class="tw-inline-block bg-main-dark color-primary tw-text-sm tw-px-2 tw-mr-1 tw-rounded-md">
        ...
        <transition :name="(openingDirection === 'up' ? 'rollup' : 'rolldown')">
            <div v-if="open" class="tw-absolute tw-left-0 tw-z-50 tw-text-white bg-base tw-cursor-pointer tw-border border-gray-02 tw-rounded-md tw-max-w-500" :class="{'tw-bottom-full': openingDirection === 'up', 'tw-top-full': openingDirection === 'down'}">
                <div v-if="searchable && openingDirection === 'down'" class="tw-px-4 tw-mb-2 tw-mt-1">
                ...
                <div class="tw-flex tw-flex-row" :id="`dropdowntab${elementID}`">
                        ...
                        <template v-if="!grouped">
                            <div v-for="(option, index) of options" :key="option.value" @click="selectEvent(option, 'select', index);">
                                ...
                                <template v-else-if="searchResults.includes(index)">
                                    <div @mouseover="searchFocus = index" class="tw-px-4 tw-py-2 tw-text-sm color-primary item tw-shadow-sm tw-outline-none entry-text-behavior" :class="[`elem-${index}`, {'hover-color': index === searchFocus, 'bg-gray-03': (lastSelectedIndex === index && !noValueSelected), 'bg-main-dark': multiSelectIndices.includes(index)}]" @mouseenter="descriptionIndex = index; showDescription = true;">
                                        {{ option[labelBy] }}
                                    </div>
                                </template>
                            </div>
                            ...
                            <div v-for="(option, index) of options" :key="option.value" @click="selectEvent(option, 'select', index);">
                                <template v-if="!currentlySearching">
                                    ...
                                    <template v-if="option.group">
                                        <div @click.capture.stop class="tw-px-4 tw-py-2 tw-font-semibold tw-text-md bg-main color-highlight tw-shadow-sm tw-outline-none entry-text-behavior" :class="[`elem-${index}`, {'hover-color': index === hasFocus}, option.classNameSpec, option.classNameGen]">
                                            {{ option[labelBy] }}
                                        </div>
                                    </template>
                                ...
                                <template v-else-if="searchResults.includes(index)">
                                    ...
                                    <template v-if="option.group">
                                        <div @click.capture.stop class="tw-px-4 tw-py-2 tw-font-semibold tw-text-md bg-main color-highlight tw-shadow-sm tw-outline-none entry-text-behavior" :class="[`elem-${index}`, {'hover-color': index === searchFocus}, option.classNameSpec, option.classNameGen]">
                                            {{ option[labelBy] }}
                                        </div>
                                    </template>
                                    ...
                                    <template v-else>
                                        <div :style="`background-color: ${optionColors[index]}`" class="tw-px-4 tw-py-2 tw-text-sm color-primary item tw-shadow-sm tw-outline-none entry-text-behavior" :class="[`elem-${index}`, {'hover-color': index === searchFocus}, option.classNameSpec, option.classNameGen, {'bg-main-dark': (lastSelectedIndex === index), 'text-blue-300': multiSelectIndices.includes(index)}]" @mouseenter="descriptionIndex = index; showDescription = true;">
                                            {{ option[labelBy] }}
                                        </div>
                                    </template>
                                </template>
                    ...
                    <div v-if="optionDescription && showDescription" class="tw-absolute tw-rounded-md tw-left-full dropdown-params tw-max-w-250 tw-text-left tw-px-4 tw-py-2 tw-whitespace-normal tw-overflow-y-scroll bg-primary">
                        <p class="tw-pb-2">{{ options[descriptionIndex].label }}</p>
        ...
        setup(props, {
            ...
            const arrowNavigation = (evt) => {
                ...
                if (currentlySearching.value && searchResults.length > 0) {
                    ...
                    if (evt.code !== "ArrowUp" && evt.code !== "ArrowDown" && evt.code !== "Enter" && open.value) {
                        ...
                        evt.preventDefault();
                        try {
                            var container = document.getElementById(`dropdowntab${elementID}`);
                            var middle = document.getElementsByClassName(`elem-${searchFocus.value}`)[0];
                    ...
                    if (evt.code !== "ArrowUp" && evt.code !== "ArrowDown" && evt.code !== "Enter" && open.value) {
                        ...
                        try {
                            var container = document.getElementById(`dropdowntab${elementID}`);
                            var middle = document.getElementsByClassName(`elem-${hasFocus.value}`)[0];
            ...
            const determineGroupedIndex = (index) => {
                ...
                props.options.every((el, index) => {
                    if (el[props.labelBy] === element[props.labelBy]) {
                        idx = index;
                        return false;
                    }
